import { useState } from 'react';
import { MessageCircle, Zap, Shield, Globe } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import '../../styles/auth.css';

const GOOGLE_SVG = (
  <svg className="google-icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
    <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
    <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
    <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
  </svg>
);

const FEATURES = [
  { icon: <Zap size={14} />, text: 'Real-time messaging with zero delay' },
  { icon: <Shield size={14} />, text: 'Secure Google authentication' },
  { icon: <Globe size={14} />, text: 'Multiple rooms to join & create' },
];

const Login = () => {
  const { signInWithGoogle, error, setError } = useAuth();
  const [loading, setLoading] = useState(false);

  const handleGoogleSignIn = async () => {
    setLoading(true);
    setError(null);
    try {
      await signInWithGoogle();
    } catch {
      // error is already set in AuthContext
    } finally {
      setLoading(false);
    }
  };

  return (
    <div className="auth-page">
      {/* Animated background */}
      <div className="auth-bg">
        <div className="auth-bg-orb auth-bg-orb-1" />
        <div className="auth-bg-orb auth-bg-orb-2" />
        <div className="auth-bg-orb auth-bg-orb-3" />
      </div>
      <div className="auth-grid" />

      {/* Auth Card */}
      <div className="auth-card">
        {/* Logo */}
        <div className="auth-logo">
          <div className="auth-logo-icon">
            <MessageCircle size={24} />
          </div>
          <span className="auth-logo-text">ChatSphere</span>
        </div>

        <h1 className="auth-heading">Welcome back</h1>
        <p className="auth-subheading">
          Sign in to join rooms, send messages,<br />and connect in real time.
        </p>

        {/* Error */}
        {error && (
          <div className="auth-error" role="alert">
            {error}
          </div>
        )}

        {/* Google Sign In */}
        <button
          id="google-signin-btn"
          className="btn-google"
          onClick={handleGoogleSignIn}
          disabled={loading}
          aria-label="Sign in with Google"
        >
          {loading ? (
            <div className="spinner spinner-sm" />
          ) : (
            GOOGLE_SVG
          )}
          {loading ? 'Signing in…' : 'Continue with Google'}
        </button>

        {/* Features */}
        <div className="auth-features">
          {FEATURES.map(({ icon, text }) => (
            <div className="auth-feature-item" key={text}>
              <div className="auth-feature-icon">{icon}</div>
              <span>{text}</span>
            </div>
          ))}
        </div>

        {/* Terms */}
        <p className="auth-terms">
          By signing in, you agree to our Terms of Service<br />and Privacy Policy.
        </p>
      </div>
    </div>
  );
};

export default Login;
